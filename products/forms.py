...
class ProductFilterForm(forms.Form):
    ...
    max_price = forms.DecimalField(
        ...
        max_digits=10,
        label='Max Price')
    min_discount = forms.DecimalField(
        required=False,
        decimal_places=2,
        max_digits=5,
        label='Min Discount')
        ...
        if self.is_valid():
            if self.cleaned_data['min_price']:
                queryset = queryset.filter(
                    price__gte=self.cleaned_data['min_price'])
            if self.cleaned_data['max_price']:
                queryset = queryset.filter(
                    price__lte=self.cleaned_data['max_price'])
            if self.cleaned_data['min_discount']:
                queryset = queryset.filter(
                    discount_percentage__gte=self.cleaned_data['min_discount'])
            if self.cleaned_data['max_discount']:
                queryset = queryset.filter(
                    discount_percentage__lte=self.cleaned_data['max_discount'])
            if self.cleaned_data['min_rating']:
                queryset = queryset.filter(
                    average_rating__gte=self.cleaned_data['min_rating'])
            if self.cleaned_data['max_rating']:
                queryset = queryset.filter(
                    average_rating__lte=self.cleaned_data['max_rating'])
        return queryset
